...
Contains everything the requirements demand: timestamps (`T=` in `TIMESCALE` units), failed pin names, failed vector numbers (`CYCLE n`), expected-vs-actual states, and per-compare PASS/FAIL flags, plus a per-block `FAILSUMMARY` whose count is *cross-checked* against actual FAIL lines (real datalogs contain such inconsistencies; we detect them).
