...
|---|---|---|
| **PySTDF** (`cmars/pystdf`) | Event-based stream reader for binary STDF **V4 only**; emits records (PTR/FTR/PRR…) as ATDF text or CSV | Record extraction, and **no `STR` support at all**, since V4-2007 postdates it. Effectively unmaintained. Also **GPL-2**, which is why an MIT-licensed project cannot vendor it and writes its own reader instead. |
| **Semi-ATE/STDF** (and Semi-ATE Metis analytics) | Maintained open-source STDF library and analytics stack. **Does cover V4-2007, `STR` included** | The honest gap here is *rendering*, not coverage: it decodes `STR` into records and dataframes and draws nothing per-cycle. Population and parametric analytics altitude; no per-pin waveform reconstruction, no expected-vs-captured differential view. |
| **STDF-Viewer** (noonchen) | PyQt/pyqtgraph GUI with a Rust-accelerated STDF **V4** parser; trend charts, histograms, Cpk stats, wafer maps, bin summaries, Excel export; last release Dec 2022 | The strongest open-source *parametric* viewer — but its own docs state FTR functional records are flattened to a pass/fail flag "used as the test value", and there is no `STR` path. The per-pin/per-vector fail payload is discarded: no waveforms, no expected-vs-actual, no signature clustering. Population altitude, not device FA. |
...
(PySTDF, Semi-ATE STDF/Metis, STDF-Viewer, GTKWave, WaveDrom, wafer-map/yield
tooling; audited July 2026) **renders** per-pin/per-cycle expected-vs-captured
data as a differential waveform, nor clusters it into fail signatures.

State the claim precisely, because the imprecise version is false. The data has
been in the standard since **STDF V4-2007**: the `STR` record carries `EXP_DATA`
and `CAP_DATA` per failing pin and cycle. PySTDF cannot read it (V4 only);
Semi-ATE/STDF reads it and renders it as records; STDF-Viewer discards the
functional payload; GTKWave has no concept of an *expected* value to compare
against. **The gap is not in the format. It is between the record domain and
the waveform domain.** PySTDF-family tools own the former, GTKWave owns the
latter, and none of the audited set fuses them or speaks the diagnostic language
of an FA report.
...
4. **Grammar-as-data ingestion** — the parser is a formal Lark EBNF grammar (a
   *datalog compiler front-end*), not regex soup. Supporting another tester
   ...
   adapters stay thin. The same argument extends to binary: the STDF record
   layout lives in a declarative field-schema table
   (`parsing/stdf/records.py`), the binary analogue of the `.lark` file, so
   correcting a field's spec order is a **data edit, not a decoder change**.
5. **Fab-floor deployability** — stdlib Tkinter plus one pure-Python dependency
   ...
   modern dashboard stacks. The STDF reader holds the line: it uses stdlib
   `struct` only, so `lark` remains the sole runtime dependency, the wheelhouse
   stays at two wheels, and the zipapp gains no package data.
...
* **Differential fail-capture mode (Tier B)** — an STDF `STR` source supplies
  expected *and* captured states, but only at logged failing pin/cycles. Each
  becomes a one-cycle `WaveformSegment`, so the existing retention-gap machinery
  renders isolated point marks with hatched no-data between them. **The renderer
  needs no new code path for this**, which is the strongest available evidence
  that representing "discarded" as structurally distinct from "held" was the
  right call.
* **Sparse-capture mode (Tier C)** — when only failing-strobe captures exist and
  no expected data does, the lane renders as a **fail-capture strip**, not a
  pretend waveform.
